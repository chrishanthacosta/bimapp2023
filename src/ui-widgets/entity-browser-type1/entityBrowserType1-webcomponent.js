...
import { showSnackbar } from "../snackbar/snack-bar";

export class EntityBrowserType1Element extends HTMLElement {
...
const selectBrowserRowHandler = (e) => {
  ...
  if (e.target.entityBroserSubject) {
    ...
    if (e.currentTarget.addedObjects) {
      const foundExisting = checkExistence(e.currentTarget.addedObjects,result);
      if (!foundExisting) {
         e.currentTarget.addedObjects.push(result);
         e.currentTarget.entityBroserSubject.next({
           action: "row-added",
           result,
         });
      } else {
        showSnackbar("Already Selected!")
      }

    } else {
      e.currentTarget.addedObjects = []
      e.currentTarget.addedObjects.push(result);
       e.currentTarget.entityBroserSubject.next({
         action: "row-added",
         result,
...
const checkExistence = (existingObjs, newObj) => {
  const props = Object.keys(existingObjs[0]);
  let foundSimilar = false;

   for (let index = 0; index < existingObjs.length; index++) {
     const obj = existingObjs[index];
     let allPropsSame = true;

     for (let index1 = 0; index1 < props.length; index1++) {
       const prop = props[index1];
       if (obj[prop] != newObj[prop]) {
         allPropsSame = false
         break;
       }
...
const getSearchParamsFromInputs = (searchInputRefs) => {
  ...
  searchInputRefs.forEach((searchInputRef) => {
    ...
    console.assert(searchInputRef[dbfieldname], "getSearchParamsFromInputs-error-"+ dbfieldname);
...
window.customElements.define("entity-browsert1", EntityBrowserType1Element);
